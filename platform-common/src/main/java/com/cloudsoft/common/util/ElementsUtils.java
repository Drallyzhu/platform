...
public class ElementsUtils {
	private static Logger logger = LoggerFactory.getLogger(ElementsUtils.class);
	...
	public static boolean isOneBlank(Object ...objects) {
		if (ElementsUtils.isNotBlank(objects)) {
			for (Object object : objects) {
				if (ElementsUtils.isBlank(object)) {
					return true;
				}
			}
			return false;
	 ...
	 * @param arr
	 */
	public static String checkIsRepeat(String arr){
		String[] strs = arr.split(",");
		boolean flag = true;   //假设不重复
		String value = null;
		if(arr == null || strs.length<=0){
			return value;
		}
		for(int i = 0;i < strs.length-1;i++){ //循环开始元素
			for(int j = i + 1;j < strs.length;j++){ //循环后续所有元素
				//如果相等，则重复
